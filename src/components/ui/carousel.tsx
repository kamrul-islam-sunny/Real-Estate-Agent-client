...
function CustomCarouselItem({ className, ...props }: React.ComponentProps<"div">) {
  const { orientation } = useCarousel()
...
function CustomCarouselPrevious2({
  className,
  variant = "outline",
  size = "icon",
  ...props
  ...
  return (
    <Button
      ...
      size={size}
      className={cn(
        "absolute size-8 rounded-full cursor-pointer border-0 bg-none z-50",	
        orientation === "horizontal"
          ? "-bottom-2 sm:right-32 right-28 -translate-y-1/2"
          : "-top-12 left-1/2 -translate-x-1/2 rotate-90",
        className
      )}
      disabled={!canScrollPrev}
  ...
  return (
    <Button
      ...
      className={cn(
        "absolute size-8 rounded-full cursor-pointer border-0 bg-none z-50",
        orientation === "horizontal"
          ? "-bottom-2 right-0  -translate-y-1/2"
          : "-bottom-12 left-1/2 -translate-x-1/2 rotate-90",
        className
      )}
      disabled={!canScrollNext}
